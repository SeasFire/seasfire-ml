...
def main(args):
    ...
    dataset = ConvLstmDataset(
        ...
        transform=ConvLstmTransform(args.test_path, args.include_global, args.append_pos_as_features),
    )
    ...
    model = ConvLstmLocalGlobalModel(
        dataset.latlon_shape[0],
        dataset.latlon_shape[1],
        len(dataset.local_features) + (4 if args.append_pos_as_features else 0),
        args.hidden_channels,
        [(3,3),(3,3)],
        len(args.hidden_channels),
        dataset.global_latlon_shape[0],
        dataset.global_latlon_shape[1],
